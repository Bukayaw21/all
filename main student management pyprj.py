...
def delete_data(rollno):
    conn = connection
    cursor = conn.cursor()
    cursor.execute('DELETE FROM pystudent WHERE rollno = %s', (rollno.get(),))
    conn.commit()
    cursor.close()
...
update_btn = tk.Button(btn_frame, bg="fuchsia", text="Update", font=("Arial", 13), width=13, command=lambda: update_data(
    rollno.get(), name.get(), gender.get(), age.get(), dep.get(), section.get(), address.get(), pob.get(), phone.get()))
